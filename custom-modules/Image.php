...
class WPC_ET_Builder_Module_Image extends ET_Builder_Module {
	function init() {
		$this->name       = esc_html__( 'WPC Image', 'et_builder' );
        $this->slug       = 'et_pb_image_wpc';
		$this->vb_support = 'off';
	...
	function render( $attrs, $content = null, $render_slug ) {
        ...
        global $wpdb;
        $attachment = $wpdb->get_col($wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE guid='%s';", esc_attr( $src ) ));
        $image1_alt = get_post_meta( $attachment[0], '_wp_attachment_image_alt', true );
        $image_caption = get_the_excerpt($attachment[0]);
        $image_title = get_the_title( $attachment[0] );
		...
		if ( 'on' === $is_overlay_applied ) {
			...
			$overlay_output = sprintf(
				'<span class="et_overlay%1$s"%2$s>%3$s</span>',
				( '' !== $hover_icon ? ' et_pb_inline_icon' : '' ),
                $data_icon,
                '<span class="wpc-image-caption-text">'.esc_attr( $image_caption ).'</span>'
			);
		}
		...
		$output = sprintf(
			...
			esc_attr( $src ),
            ( '' !== $alt ? esc_attr( $alt ) : esc_attr( $image1_alt ) ),
			( '' !== $title_text ? sprintf( ' title="%1$s"', esc_attr( $title_text ) ) : sprintf( ' title="%1$s"', esc_attr( $image_title ) ) ),
            'on' === $is_overlay_applied ? $overlay_output : '<span class="wpc-image-caption-text">'.esc_attr( $image_caption ).'</span>'
		);
...
function _et_bb_module_image_add_src_label_wpc( $filed ) {
...
add_filter( 'et_builder_module_fields_et_pb_image_field_src', '_et_bb_module_image_add_src_label_wpc' );

new WPC_ET_Builder_Module_Image;
